...
{
	//DC Low
	LCD_DC_GPIO_PORT->BSHR = (1 << (16 + LCD_DC_GPIO_NUM));
	glcd_spi_write(c);	
}
...
{ 
	// DC High
	LCD_DC_GPIO_PORT->BSHR = (1 << LCD_DC_GPIO_NUM);
	glcd_spi_write(c);	
}
...
void glcd_ST7565R_init(void) {
	...
	glcd_command(0xe2);//soft reset
	Delay_Ms(10);
	glcd_command(0x2c);  	//Control built-in power circuit ON	    VB
	glcd_command(0x2e);  	//Control built-in power circuit ON	    VR 
	glcd_command(0x2f);  	//Control built-in power circuit ON	    VF 
	Delay_Ms(10);
	glcd_command(0xf8);  //Set booster level
 	glcd_command(0x00);  //Set  4X booster level 

	glcd_command(0x22);  //Select regulation resistor ratio
	glcd_command(0x81);  	 //Setelectronic volume (EV) level 
	glcd_command(30);  //Setelectronic volume (EV) level 
	glcd_command(0xa2);  //1/9bias 
	glcd_command(0xc8);  //Set output direction of COM 
	glcd_command(0xa0);  //Set scan direction of SEG 
 	glcd_command(0xa4);  //Set normal display 

  	glcd_command(0xa6);  //Set normal display 

	glcd_command(0x40);  //start line
	glcd_command(0xaf);  //  display ON 
